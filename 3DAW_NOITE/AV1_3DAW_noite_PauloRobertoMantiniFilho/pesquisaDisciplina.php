...
{    
    ...
    $disciplina = (isset($_POST["consulta"])?$_POST["consulta"]:false);   

    if($operacao == "pesquisaDisciplina")
    {        
        $valores = $strcon->query("SELECT * FROM `disciplinas` WHERE nome LIKE '%".$disciplina."%'");
            ...
            for ( $i=0;$i<$linhas; $i++)
            {
                ...
                echo "<th>$reg[1]</th><th>$reg[2]</th><th>$reg[3]</th><th>$reg[4]</th><th>";
                echo '</tr>';
            }
            echo '</tbody>';
            echo '</table>';
